  ...
  (function () {
    ...
    function loadPayPalSdk() {
      return new Promise(function (resolve, reject) {
        ...
        var s = document.createElement("script");
        s.src = "https://www.paypal.com/sdk/js?client-id=" + encodeURIComponent(paypalClientId) + "&currency=" + encodeURIComponent(paypalCurrency) + "&intent=capture&components=buttons,funding-eligibility&enable-funding=card";
        s.onload = function () { resolve(); };
    ...
    function buildPayPalButtonOptions(fundingSource) {
      ...
      if (fundingSource) opts.fundingSource = fundingSource;
      return opts;
    }
    ...
    function renderPayPalButtons() {
      ...
      loadPayPalSdk().then(function () {
        var paypalFunding = window.paypal.FUNDING && window.paypal.FUNDING.PAYPAL;
        var paypalOnlyButtons = window.paypal.Buttons(buildPayPalButtonOptions(paypalFunding));
        if (!paypalOnlyButtons.isEligible()) throw new Error("PayPal checkout is not eligible for this buyer.");
        return paypalOnlyButtons.render(paypalButtons)
          .then(function () {
            if (!paypalCardButtons || !window.paypal.FUNDING || !window.paypal.FUNDING.CARD) return;
            var cardButtons = window.paypal.Buttons(buildPayPalButtonOptions(window.paypal.FUNDING.CARD));
            if (!cardButtons.isEligible()) return;
            return cardButtons.render(paypalCardButtons);
          });
      }).catch(function (err) {
